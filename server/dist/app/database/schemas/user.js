...
function onSave(next) {
  var user = this; // ensure user picture is set
  ...
  _bcryptNodejs["default"].genSalt(SALT_WORK_FACTOR, function (err, salt) {
    ...
    _bcryptNodejs["default"].hash(user.password, salt, null, function (error, hash) {
...
UserSchema.methods.validatePassword = function validatePassword(password, callback) {
  _bcryptNodejs["default"].compare(password, this.password, function (err, isMatch) {
